  ...
  function renderHome() {
    ...
    section.innerHTML = `
      <div class="cultivation-head">
        ...
        <p>境界按各板块的知识里程碑划分，不按固定百分比等分。每条修炼路线独立计算，进度仍只保存在当前浏览器。</p>
      </div>
      <div class="cultivation-grid">
        ${Object.entries(configs).map(([type,cfg]) => {
          const r = realmInfo(type);
          return `<a class="cultivation-card ${cfg.tone}" href="${cfg.href}">
